...
// 1. ADMIN AUTHORIZATION CHECK (dh = 1 required via Parameterized Statement)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 🔒 CSRF FIREWALL: Terminate request instantly if forged or missing
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        header("HTTP/1.1 403 Forbidden");
        die("CRITICAL SECURITY ERROR: CSRF MATRIX MALFUNCTION. ASSET REGISTRATION TERMINATED.");
    }
    ...
    // Capture checkboxes natively (Checked = 1, Unchecked/Missing = 0)
    $perm_modify   = isset($_POST['perm_modify']) ? 1 : 0;
    $perm_copy     = isset($_POST['perm_copy']) ? 1 : 0;
    $perm_transfer = isset($_POST['perm_transfer']) ? 1 : 0;

    if (empty($raw_uuid) || empty($raw_name) || empty($raw_type)) {
        $status_message = "ERROR: CRITICAL REGISTRY FIELD NULL EXCEPTION DETECTED.";
        $status_type = "failure";
    } else {
        // Secure Parameterized Query Structure for Parent Asset
        $AssetUpload = "INSERT INTO `assets` (`uuid`, `type`, `name`) VALUES (?, ?, ?)";
        if ($stmt_upload = $conn->prepare($AssetUpload)) {
            $stmt_upload->bind_param("sis", $raw_uuid, $raw_type, $raw_name);

            if ($stmt_upload->execute()) {
                // Fetch the auto-incremented ID generated by the parent table row write
                $new_asset_id = $conn->insert_id;
                $stmt_upload->close();

                // 🛠️ Secure Parameterized Upsert Structure for your child asset_perms table
                $perms_sql = "INSERT INTO `asset_perms` (`asset_id`, `Modify`, `Copy`, `Transfer`) 
                              VALUES (?, ?, ?, ?)
                              ON DUPLICATE KEY UPDATE `Modify` = VALUES(`Modify`), `Copy` = VALUES(`Copy`), `Transfer` = VALUES(`Transfer`)";
                
                if ($stmt_perms = $conn->prepare($perms_sql)) {
                    $stmt_perms->bind_param("iiii", $new_asset_id, $perm_modify, $perm_copy, $perm_transfer);
                    
                    if ($stmt_perms->execute()) {
                        $status_message = "OK | ASSET DATA MATRIX AND PERMISSION VECTORS COMPILED SUCCESSFULLY.";
                        $status_type = "success";
                        
                        // Fire defensive tracking log metadata metrics
                        $log_telemetry = "Registered new fleet component configuration. Designation: [".$raw_name."] // ID Matrix: [".$raw_uuid."].";
                        record_security_log($conn, $login_session, 'INSERT', 'ASSET_LOGS', $raw_uuid, $log_telemetry);
                    } else {
                        $status_message = "PARTIAL SUCCESS | ASSET INJECTED BUT PERMISSION SET METRIC FAILURE: " . htmlspecialchars($conn->error);
                        $status_type = "failure";
                    }
                    $stmt_perms->close();
                } else {
                    $status_message = "PARTIAL SUCCESS | FAILED TO PREPARE SYSTEM REPOSITORY PERMISSION WRITER.";
                    $status_type = "failure";
                }
            } else {
                $status_message = "ERROR | ASSET TELEMETRY WRITE EXCEPTION: " . htmlspecialchars($conn->error);
                $status_type = "failure";
                $stmt_upload->close();
            }
        } else {
            $status_message = "ERROR | FAILED TO PREPARE SYSTEM RECORD REGISTRY WRITER.";
            $status_type = "failure";
        }
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>RPGROUP - Asset Registry Console</title>
    <style>
        ...
        .perm-checkbox-group { display: flex; gap: 25px; align-items: center; background: #000; border: 2px solid var(--lcars-dark-blue); padding: 10px 15px; border-radius: 4px; height: 46px; box-sizing: border-box; }
        .lcars-check-label { display: flex; align-items: center; gap: 8px; color: #fff !important; cursor: pointer; font-size: 13px !important; text-transform: uppercase; }
        .lcars-check-label input { accent-color: var(--lcars-orange); width: 16px; height: 16px; margin: 0; cursor: pointer; }
    </style>
</head>
<body>
    ...
    <div class="lcars-container">
        ...
        <nav class="lcars-left-bracket">
            ...
            <div class="lcars-menu">
                ...
                <a href="dhpanel.php" class="lcars-btn btn-blue">ADMIN TERM</a>
                <a href="view_assets.php" class="lcars-btn btn-pink">VIEW ASSETS</a>
        ...
        <main class="lcars-main-panel">
            ...
            <?php if (!empty($status_message)): ?>
                <div class="telemetry-banner <?php echo ($status_type === 'success') ? 'telemetry-success' : 'telemetry-failure'; ?>">
                    DIAGNOSTIC STATUS LOG: <?php echo htmlspecialchars($status_message); ?>
                </div>
            <?php endif; ?>
            ...
            <section class="lcars-admin-section">
                ...
                <form method="POST" action="">
                            ...
                            <select id="asset_type" name="type" class="lcars-select" required>
                                ...
                                <?php if (!empty($asset_types)): ?>
                                    <?php foreach ($asset_types as $type_row): ?>
                                        <option value="<?php echo (int)$type_row['atid']; ?>">
                    ...
                    <!-- Asset Permissions Matrix Checkboxes -->
                    <div class="form-row" style="margin-bottom: 25px;">
                        <label>ASSET OPERATIONS RIGHTS OVERLAY (MODIFY / COPY / TRANSFER)</label>
                        <div class="perm-checkbox-group">
                            <label class="lcars-check-label">
                                <input type="checkbox" name="perm_modify" value="1"> MODIFY
                            </label>
                            <label class="lcars-check-label">
                                <input type="checkbox" name="perm_copy" value="1"> COPY
                            </label>
                            <label class="lcars-check-label">
                                <input type="checkbox" name="perm_transfer" value="1"> TRANSFER
                            </label>
                        </div>
                    </div>
