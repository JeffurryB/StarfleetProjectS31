...
// 🔒 CSRF INITIALIZATION MATRIX: Generate dynamic security key if not set
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 🔒 CSRF SECURITY FIREWALL: Terminate request instantly if forged or missing
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        header("HTTP/1.1 403 Forbidden");
        die("CRITICAL SECURITY ERROR: CSRF MATRIX MALFUNCTION. RECORD MUTATION REFUSED.");
    }
    ...
    // Map Numeric Gender Choices (1, 2, 3) to DB String Layout Values
    if ($raw_gender === 'Male' || $raw_gender === '1') {
        $gender = 1;
    } elseif ($raw_gender === 'Female' || $raw_gender === '2') {
        $gender = 2;
    ...
    // Capture and sanitize SJ_info service jacket fields
    $languages    = isset($_POST['languages']) ? trim($_POST['languages']) : '';
    $religion     = isset($_POST['religion']) ? trim($_POST['religion']) : '';
    $height_cm    = (!empty($_POST['height_cm'])) ? floatval($_POST['height_cm']) : null;
    $weight_kg    = (!empty($_POST['weight_kg'])) ? floatval($_POST['weight_kg']) : null;
